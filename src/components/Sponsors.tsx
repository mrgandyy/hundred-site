  ...
  return (
    <section id="sponsors" className="py-24 border-t border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* fewer columns so each card is wider */}
        <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-8">
          {sponsors.map(s => (
            <div key={s.name} className="p-8 rounded-3xl bg-base-800/70 border border-white/10">
              <div className="h-24 rounded-2xl bg-white/5 border border-white/10 grid place-items-center overflow-hidden">
                <Image
                  ...
                  width={360}
                  height={96}
                  className="max-h-20 w-auto object-contain opacity-95"
                  unoptimized
                />
              </div>
              ...
              {s.url && (
                <a
                  href={s.url}
                  target="_blank"
                  className="mt-5 inline-block text-sm px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10"
                >
